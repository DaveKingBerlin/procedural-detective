...
class GenerationFailureCode(str, Enum):
    ...
    # -- Phase 19J — typed internal ACTIVITY_LOG failures (closed vocabulary).
    # Strings EXACTLY match ``app.domain.activity_log.ActivityLogValidatorCode``
    # values so the pure validator codes map onto the canonical failure vocab
    # without a domain -> generation import. The public UI never exposes these
    # raw validator details; it maps them to the Phase19J §39 safe copy.
