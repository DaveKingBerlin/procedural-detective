...
export function summarizeDiscovery(
  ...
  return { entries, discoveredCount: entries.length };
}
 ...
 *
 * PD-SEC-01 (Phase 20): the old "Discovered X / Y evidence items" form used
 * the pre-reveal count of evidence-linked world objects as the denominator.
 * The backend now strips that linkage (evidenceId) from undiscovered objects,
 * so the denominator is structurally unknowable on the client — the player
 * sees a bare, always-honest "Discovered X evidence items".
 */
